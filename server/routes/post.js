...
router.get('/get_friend_posts', function(req, res){
    query ="SELECT u.username, u.origin, post.post_content, post.id, post.date_posted FROM  users u INNER JOIN user_friends friend ON (u.id = friend.friend_id) INNER JOIN user_posts post ON (post.user_id = friend.friend_id) WHERE friend.user_id=" + req.user_id;
